...
object KartMountEvents {
     ...
     * 첫 어트리뷰트가 아직 적용되지 않았을 수 있으므로 [Player.ridingKart]와
     * [KartSaddle.kart]가 모두 `null`일 수 있습니다.
     */
    @JvmField
     ...
     *
     * 렌더 스레드에서 호출되며, 이 시점에는 [Player.ridingKart]와 [KartSaddle.kart]로
     * 준비된 카트에 접근할 수 있습니다.
     */
    @JvmField
     ...
     * 관계에서는 이 콜백이 끝날 때까지 [Player.ridingKart]와 [KartSaddle.kart]로 준비된
     * 카트에 접근할 수 있습니다.
     *
     ...
     *
     * 카메라 대상 변경이 실제로 반영된 직후 렌더 스레드에서 호출됩니다. 대상 카트의 첫
     * 어트리뷰트가 아직 준비되지 않았다면 [KartSaddle.kart]는 `null`일 수 있습니다.
     */
    @JvmField
     ...
     * 호출되며 [KartSaddle.kart]로 준비된 카트에 접근할 수 있습니다.
     */
    @JvmField
    ...
    fun interface KartSpectateEndCallback {
        /**
         * @param kartEntity 이전 관전 대상이 탑승했던 카트의 대구 엔티티
