 ...
 *
 * ## API 안정성
 * 마크라이더 데이터팩의 구현 세부사항을 직접 반영하므로 의도적으로 unstable 상태를
 * 유지합니다. 카트 상태 정보의 제공은 계속 지원하지만 개별 필드, modifier ID와 값의
 * 의미는 패치 버전에서도 호환성 없이 변경될 수 있습니다.
 *
 * 실제 모드에서 이 값을 직접 사용한다면 SkidMC 업데이트 시 버전별 변경 사항을 확인하고
 * 각 modifier의 의미와 값 범위를 다시 검토해야 합니다.
 *
