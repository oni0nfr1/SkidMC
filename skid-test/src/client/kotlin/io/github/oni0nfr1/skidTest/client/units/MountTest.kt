...
object MountTest: TestUnit() {
    ...
    override val description: String = """
        ...
        7. 관전 대상을 변경하거나 관전을 종료하고, SPECTATE_END 메시지가 정상적으로 출력되는지 확인한다.
    ...
    override fun test(): TestResult {
        ...
        KartMountEvents.SPECTATE_END.register { kartEntity, rider, target ->
            if (!status.testing) return@register
            client.sendChat("[SPECTATE_END] ${rider.name.string} (player#${rider.id}) is not spectating ${target.name.string} (player#${target.id}) riding kart#${kartEntity.id}.")
        }
